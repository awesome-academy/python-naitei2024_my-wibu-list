"""
Test info:
    test_anime_detail_view_context_data: Tests if all context data is correctly
        populated.
    test_anime_detail_view_random_button_functionality: Tests if the random
        button returns a valid Content instance.
    test_anime_detail_view_pagination: Tests if pagination works correctly
        for comments.
    test_anime_detail_view_no_products: Tests behavior when there are no
        products related to the content.
    test_anime_detail_view_user_favorite_status: Tests the favorite status
        functionality for a logged-in user.
"""
...
class AnimeDetailViewTests(TestCase):
    ...
    def test_anime_detail_view_with_logged_in_user(self):
        ...
        self.assertContains(response, self.favorite.cid.name)
        self.assertContains(response, '8')  # Check if the score is correctly displayed
    ...
    def test_anime_detail_view_without_logged_in_user(self):
        ...
        self.assertContains(response, self.content1.name)
        self.assertContains(response, self.comment.content)
        self.assertNotContains(response, '8')  # No score should be shown for anonymous user
    ...
    def test_anime_detail_view_context_data(self):
        ...
        self.assertEqual(context["favorite"], self.favorite)
    ...
    def test_anime_detail_view_user_favorite_status(self):
        ...
        response = self.client.get(reverse('anime_detail', kwargs={'pk': self.content1.cid}))
        self.assertEqual(response.context['favorite'], self.favorite)  # Favorite should be present in context
